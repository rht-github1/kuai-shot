...
- 选区后长图：先用蓝框确认选区，进入后遮罩让开一次；在原区域滚动，控制条持续跟拍，保存 / ✓ 与平时相同（不 OCR）
- 颜色、线宽、撤销 / 重做
- 复制到剪贴板（关窗后再写入，避免 Wayland 把内容清掉）；只有点保存才会写文件
- 钉在最上层，可拖动，右键或双击关闭
- 原图像素导出，不二次压缩、不拉伸降质
...
| 序号钉 | 点击放置 1、2、3… |
| 长图 | 画选区确认蓝框后点「长图」或框内滚动；遮罩让开，控制条记帧；滚动时蓝框不留在页面上；保存 / ✓ 与平时相同 |
| 右键 | 取消 |
| 📌 | 复制并钉在最上层；钉图可拖，右键或双击关闭 |

托盘图标单击也可开始截图。保存前不会往「图片」目录自动落盘。

长图：先画选区确认蓝框（可在框里滚轮，或点「长图」）。进入后全屏遮罩只让开一次，不再闪回来；在刚才那块页面上自己滚动。控制条永不隐藏，预览变了就是记下了新帧。滚动时蓝框不会留在页面上。保存会写文件并复制；✓ 只复制。滚太快仍可能漏中间一截，尽量匀速滚。
...
覆盖 1 / 2 / 3 屏布局、跨屏拼接、窗口对位、长图重叠拼接。托盘仍用 Qt，遮罩仍用 GTK，不合成一套 toolkit（Wayland 缩放下两套窗口的 DPR 不一致）。
